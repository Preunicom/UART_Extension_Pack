--! @file
--! @brief Testbench for the SPI_Buffer_Register_Serializer
--! @details
--! This file contains the testbench for the SPI_Buffer_Register_Serializer entity.
--! It tests:
--! - Normal operation
...
entity TB_SPI_Buffer_Register_Serializer is
  Port (
    tb_error : out std_logic --! '0' if everything works like expected, '1' otherwise.
  );
end TB_SPI_Buffer_Register_Serializer;

architecture TESTBENCH of TB_SPI_Buffer_Register_Serializer is
...
begin
    COMP: SPI_Buffer_Register_Serializer generic map(8) port map(tb_clk, tb_rst, tb_write_enable, tb_data_in, tb_data_not_needed_anymore, tb_data_out, tb_full);
