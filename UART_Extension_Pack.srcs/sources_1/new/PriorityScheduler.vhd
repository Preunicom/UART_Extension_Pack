  ...
  begin
    if rising_edge(clk) then
      if rst = '1' then
        ...
        outp_valid <= '0';
        case state is
          when S0 => -- schedule and wait for Encoder to apply schedule data
            if last_schedule_next = '1' and schedule_next = '0' then
              -- Encoder applied data
              --> get new schedule requirements from units
              state <= S1;
              scheduler_done <= next_outputs;
            else
              -- Encoder did not apply data
              --> schedule to get up to date data
              state <= S0;
              outp_valid <= '1';
              -------------------- SCHEDULE --------------------
              ...
              if scheduler_wanted(0) = '1' then
                ...
                -- No requirement for scheduler
                outp_valid <= '0';
                control_sig <= "000000";
              end if;
              ------------------ SCHEDULE END ------------------
            end if;
          when S1 => -- waiting for Encoder to be ready
            if last_schedule_next = '0' and schedule_next = '1' then
              -- Encoder ready
              state <= S0;
            else
              -- Wait
