...
architecture Behavioral of GPIO_Wrapper is
  ...
  --! Current GPIO input values extended to 14 bits.
...
begin
  GPIO: GPIO_Bank_Unit
    generic map(IN_PINS, OUT_PINS)
    port map(
      clk => clk,
      rst => rst,
      write_en => write_mode_en,
      config_in => write_values,
      values_out => values_read(IN_PINS-1 downto 0),
